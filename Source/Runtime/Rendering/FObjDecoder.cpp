	...
	while (true)
	{
		...
		else if (Type == "f")
		{
			...
			for (uint8 i = 0; i < MAX_VERTEX_INDICES_COUNT; ++i)
			{
				...
				{
					OutModelData->Indices.push_back(UniqueVertexIndexIter->second);
				...
				uint32_t NewIndex = static_cast<uint32_t>(OutModelData->Vertices.size());
				OutModelData->Vertices.push_back(NewVertexData);
				OutModelData->Indices.push_back(NewIndex);
